...
class Upsample(nn.Module):
    ...
    def forward(self, x, target_size, inference=False):
        ...
        if inference:
            ...
            # W = x.size(3)
            #
            # x = x.view(B, C, H, W, 1).expand(B, C, H, target_size[2] // H, target_size[3] // W)
            # x = x.contiguous().view(B, C, target_size[2], target_size[3])
...
class Neck(nn.Module):
    ...
    def forward(self, input, downsample4, downsample3, inference=False):
        x1 = self.conv1(input)
        x2 = self.conv2(x1)
        x3 = self.conv3(x2)          # [b,512,13,13]

        # SPP
        m1 = self.maxpool1(x3)       # [b,512,13,13]
        m2 = self.maxpool2(x3)       # [b,512,13,13]
        m3 = self.maxpool3(x3)       # [b,512,13,13]
        spp = torch.cat([m3, m2, m1, x3], dim=1)     # [b,2048,13,13]

        x4 = self.conv4(spp)
        x5 = self.conv5(x4)
        x6 = self.conv6(x5)
        x7 = self.conv7(x6)         # [b, 256, 13, 13]

        # UP
        up = self.upsample1(x7, downsample4.size(), self.inference) # [b, 256, 26, 26]

        # R 85
        x8 = self.conv8(downsample4)        # [b, 256, 26, 26]
        # R -1 -3
        x8 = torch.cat([x8, up], dim=1)     # [b, 512, 26, 26]
        ...
        x12 = self.conv12(x11)
        x13 = self.conv13(x12)
        x14 = self.conv14(x13)              # [b, 128, 26, 26]
...
class Yolov4(nn.Module):
    def __init__(self, yolov4conv137weight=None, n_classes=80, inference=False):
        ...
        if yolov4conv137weight:
            self.init_backbone_weight(yolov4conv137weight)
    ...
    def init_backbone_weight(self, yolov4conv137weight):
