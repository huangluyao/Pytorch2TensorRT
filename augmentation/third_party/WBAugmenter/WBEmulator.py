...
class WBEmulator:
    def __init__(self):
        ...
        self.features = np.load('./augmentation/third_party/WBAugmenter/params/features.npy') # training encoded features
        self.mappingFuncs = np.load('./augmentation/third_party/WBAugmenter/params/mappingFuncs.npy') # mapping functions to emulate WB effects
        self.encoderWeights = np.load('./augmentation/third_party/WBAugmenter/params/encoderWeights.npy') # weight matrix for histogram encoding
        self.encoderBias = np.load('./augmentation/third_party/WBAugmenter/params/encoderBias.npy') # bias vector for histogram encoding
    ...
    def rgbUVhist(self, I):
        ...
        for i in range(3):  # for each histogram layer, do
            ...
            diff_u = np.abs(np.expand_dims(Iu,axis=1) - np.expand_dims(A,axis=0))
            diff_v = np.abs(np.expand_dims(Iv,axis=1) - np.expand_dims(A,axis=0))
            ...
            temp = Iy* (diff_u).transpose()

            print("run time numpy mul %.4f s" % (time.time() - start))
